...
> **注意**：加载数据的时候，我们直接�?PassengerID （乘客编号）列处理成了行索引�?
...
| `Name`     | 乘客姓名         | 字符�?                                             |
| `Sex`      | 性别             | 字符串，`male`为男性，`female`为女�?               |
| `Age`      | 年龄             | 浮点�?                                             |
| `SibSp`    | 平辈亲属登船人数 | 整数，包括兄弟、姐妹、配偶总共登船的人�?           |
| `Parch`    | 父母小孩登船人数 | 整数                                                |
| `Ticket`   | 船票�?          | 字符�?                                             |
| `Fare`     | 船票价格         | 浮点�?                                             |
| `Cabin`    | 客舱�?          | 字符�?                                             |
...
3. **特征选择**：从大量的特征中筛选出对模型预测有用的特征，去除冗余或无关的特征，减少维度并提高模型的性能。常用的方法有：
    ...
    - **过滤�?*：基于统计检验方法（如：卡方检验、皮尔逊相关系数等）选择最具区分度的特征�?
    - **包裹�?*：通过模型的性能来评估特征子集的效果（如：递归特征消除法）�?
    - **嵌入�?*：通过训练模型并从模型中提取特征的重要性（如：决策树、L1正则化等），从而进行特征选择�?
...
4. **特征降维**：通过某种数学变换将数据从高维空间映射到低维空间，以便保留数据的主要信息，简化模型，降维后的新特征是原始特征的某种组合。降维可以减少模型训练的运算量，减少不必要的噪声和冗余特征来降低复杂性，从而提高模型的泛化能力。特征降维常用的方法有：
    ...
    - **LDA**（线性判别分析）：LDA 的主要目标是通过寻找最佳的投影方向来最大化不同类别之间的可分性，并同时最小化同一类别内数据点的散布。LDA在特征空间中找到一个低维的空间，使得类别之间的差异尽可能大，而同一类别内的数据尽量聚集。简单的说，LDA 的核心思想�?*最大化类间散度，最小化类内散度**。Scikit-learn 库`discriminant_analysis`模块的`LinearDiscriminantAnalysis`类可以帮助我们完成线性判别分析�?
...
接下来，还需要对性别和登船港口两个字段进行独热编码处理，可以使用 pandas 库中的`get_dummies`函数�?scikit-learn 库的`OneHotEncoder`处理，两者处理的结果类似，代码如下所示�?
...
首先我们将数据集划分为训练集和验证集，注意这里不是划分训练集和测试集，因为我们最终的测试集是官方提供的`test.csv`文件，这里我们把`train.csv`�?90% 的数据划分出来训练模型，用剩下的 10% 对模型进行验证。下面的代码中我们将验证集的特征和标签分别命名为`X_valid`和`y_valid`�?
...
params = {
    ...
    'objective': 'binary:logistic',  # 指定模型的损失函�?
...
接下来我们加载真正的测试数据`test.csv`，通过前面训练好的模型来做出预测。我们可以将预测的结果保存成一�?CSV 文件，该文件共有两列，一列是 PassengerID，一列是我们预测的结果。我们将该文件提交到 Kaggle 平台，可以获得最终模型的准确率评分�?
...
4. 对名字（Name）中�?Mrs 且父母小孩登船人数大�?的（可能是一位母亲）单独处理（如增加一个布尔字段）�?
...
> **说明**：此处我们默认上面代码中的`X_test`已经处理�?XGBoost 中`DMatrix`对象�?

如果我们希望将模型部署成 Web 服务，可以利�?Flask、FastAPI 这样�?Web 框架来创�?API 接口。下面，我们�?Flask 框架为例，用简单的代码为大家演示这个过程�?
...
> **说明**：作为工程化的项目，肯定不能在请求到来时才加载模型，因为这样会严重影�?Web 服务的性能。可以在项目启动之后的适当时机提前加载需要的模型，与此同时还要根据实际需求做好模型资源占用的监控�?

运行上面的代码，默认会在本机 5000 端口运行一�?Web 服务器，下面我们通过 API 测试工具模拟发送一�?HTTP 请求给服务器，看看我们的模型能否运转起来给出预测的结果�?
