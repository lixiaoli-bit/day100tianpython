...
1. `HTTPRequest`对象的属性和方法�?
   ...
   - `method` - 获取请求方法
   - `path` / `get_full_path()` - 获取请求路径/带查询字符串的路�?
   - `scheme` / `is_secure()` / `get_host()` / `get_port()` - 获取请求的协�?主机/端口
   - `META` / `COOKIES` - 获取请求�?Cookie信息
   - `GET` / `POST` / `FILES` - 获取GET或POST请求参数/上传的文�?
...
3. `HttpResponse`对象的属性和方法�?
   ...
   - `__setitem__` / `__getitem__` / `__delitem__` - 添加/获取/删除响应�?
   - `charset` / `content` / `status_code` - 响应的字符集/消息体（bytes流）/状态码
     ...
     - 5xx（服务器错误）：服务器处理请求失败�?
4. `JsonResponse`（`HttpResponse`的子类型）对�?
...
问题2：关系型数据库中数据完整性指的是什么？什么时候需要牺牲数据完整性？（实体完整�?参照完整�?域完整性）

问题3：ORM是什么以及解决了什么问题？（对象模�?关系模型双向转换�?
...
4. `QuerySet`的方�?
   ...
   - `filter()` / `exclude()`
     ...
     - `exact` / `iexact`：精确匹�?忽略大小写的精确匹配查询
     - `contains` / `icontains` / `startswith / istartswith / endswith / iendswith`：基于`like`的模糊查�?
     - `in`：集合运�?
     - `gt` / `gte` / `lt` / `lte`：大�?大于等于/小于/小于等于关系运算
     - `range`：指定范围查询（SQL中的`between…and…`�?
     - `year` / `month` / `day` / `week_day` / `hour` / `minute` / `second`：查询时间日�?
     - `isnull`：查询空值（`True`）或非空值（`False`�?
     - `search`：基于全文索引的全文检�?
     - `regex` / `iregex`：基于正则表达式的模糊匹配查�?
     - `aggregate()` / `annotate()`
...
5. `Q`对象和`F`对象
   ...
   ... ) # 查询名字以“张”开头且工资小于等于5000或补贴大于等�?000的员�?
   <QuerySet [<Emp: 张三�?]>
   ```
...
2. 用户的请求可能会包含多个（持久化）操作，这些操作有可能需要设计成不可分割的原子性操作，那么这里就形成了事务的边界�?
   ...
   - 数据并发访问会产�?种问题（请参考我的[《Java面试题全集（上）》](https://blog.csdn.net/jackfrued/article/details/44921941)�?0题对该问题的讲解）：

     - �?类丢失更新（A事务撤销覆盖B事务更新的数据）和第2类丢失更新（A事务提交覆盖B事务更新的数据）�?
...
1. MIME（多用途Internet邮件扩展）类�?- 告知浏览器传输的数据类型�?
...
3. 生成CSV / Excel / PDF / 统计报表�?
   ...
   - 大文件的流式处理：`StreamingHttpResponse`�?
     ...
     def download_file(request):
         file_stream = open('...', 'rb')
         # 如果文件的二进制数据较大则最好用迭代器进行处理避免过多的占用服务器内�?
   ...
   - [ECharts](http://echarts.baidu.com/)或[Chart.js](https://www.chartjs.org/)�?
     ...
     def get_charts_data(request):
         ...
         with connections['backend'].cursor() as cursor:
             # 在使用ORM框架时可以使用对象管理器的aggregate()和annotate()方法实现分组和聚合函数查�?
             # 执行原生SQL查询(如果ORM框架不能满足业务或性能需�?
...
问题1：中间件背后的设计理念是什么？（分离横切关注功�?拦截过滤器模式）

问题2：中间件有哪些不同的实现方式？（参考下面的代码�?

问题3：描述Django内置的中间件及其执行顺序。（推荐阅读：[Django官方文档 - 中间�?- 中间件顺序](https://docs.djangoproject.com/zh-hans/2.0/ref/middleware/#middleware-ordering)�?
...
1. CommonMiddleware - 基础设置中间�?
   - DISALLOWED_USER_AGENTS - 不被允许的用户代理（浏览器）
   - APPEND_SLASH - 是否追加`/`
   - USE_ETAG - 浏览器缓存相�?

2. SecurityMiddleware - 安全相关中间�?
   - SECURE_HSTS_SECONDS - 强制使用HTTPS的时�?
   - SECURE_HSTS_INCLUDE_SUBDOMAINS - HTTPS是否覆盖子域�?
   - SECURE_CONTENT_TYPE_NOSNIFF - 是否允许浏览器推断内容类�?
   - SECURE_BROWSER_XSS_FILTER - 是否启用跨站脚本攻击过滤�?
   - SECURE_SSL_REDIRECT - 是否重定向到HTTPS连接
   - SECURE_REDIRECT_EXEMPT - 免除重定向到HTTPS

3. SessionMiddleware - 会话中间�?

4. CsrfViewMiddleware - 防范跨站身份伪造中间件

5. XFrameOptionsMiddleware - 防范点击劫持攻击中间�?
...
5. session的序列化�?
   ...
   - JSONSerializer�?.6及以后默认）- 如果想将自定义的对象放到session中，会遇到“Object of type 'XXX' is not JSON serializable”的问题（如果配置使用Redis保存Session，django-redis使用了Pickle序列化，这个问题就不存在了）�?
   - PickleSerializer�?.6以前的默认）- 因为安全问题不推荐使用，但是只要不去反序列化用户构造的恶意的Payload其实也没有什么风险。关于这种方式的安全漏洞，可以参考《[Python Pickle的任意代码执行漏洞实践和Payload构造》](http://www.polaris-lab.com/index.php/archives/178/)一文或《软件架�?Python语言实现》上关于这个问题的讲解�?
...
2. 实时日志文件分析：Python + 正则表达�?+ Crontab
...
问题2：如何解决多个JavaScript库之间某个定义（�?函数）冲突的问题�?
...
<script>
    ...
    var $btn = $('#okBtn');     // jQuery对象(拥有更多的属性和方法而且没有浏览器兼容性问�?
    $btn.on('click', function() {});
    // $(btn)可以将原生JavaScript对象转成jQuery对象
    // $btn.get(0)�?btn[0]可以获得原生的JavaScript对象
</script>
```
...
3. 创建Celery实例�?
   ...
   # 让Celery自动从参数指定的应用中发现异步任�?定时任务
   # app.autodiscover_tasks(['common', ])
   # 让Celery自动从所有注册的应用中发现异步任�?定时任务
...
6. 创建定时任务�?
   ...
   app.conf.update(
       ...
       enable_utc=True,
       # 定时任务（计划任务）相当于是消息的生产�?
       # 如果只有生产者没有消费者那么消息就会在消息队列中积�?
       # 将来实际部署项目的时候生产者、消费者、消息队列可能都是不同节�?
       beat_schedule={
           'task1': {
...
2. 记得�?Python 代码放在 Web 服务器的文档根目录之外，避免代码意外泄露�?
3. 谨慎处理用户上传的文件�?
...
可以使用nose2或pytest来辅助执行单元测试，同时通过cov-core或pytest-cov可以对测试覆度进行评估。覆盖率由百分比表示。比如测试代码执行过了程序的每一行，那么覆盖率就�?00%。这种时候，几乎不会出现新程序上线后突然无法运行的尴尬情况。覆盖率不关心代码内容究竟是什么，覆盖率是用来检查“测试代码不足、测试存在疏漏”的一个指标，“测试内容是否妥当”并不归它管�?
...
1. 数据源：数据本地�?/ 置于内存�?/ 测试之后回滚

2. 资源虚拟化：存根/桩（stub）、仿�?模拟（mock）、伪造（fake�?

   - stub：测试期间为提供响应的函数生成的替代�?
...
集成多个函数或方法的输入输出的测试，测试时需要将多个测试对象组合在一起�?
 ...
 - 测试组件互操作�?/ 需求变更测�?/ 外部依赖和API / 调试硬件问题 / 在代码路径中发现异常
...
5. 项目上线之前，请记住**去掉django-debug-toolbar相关的所有配�?*�?
...
2. 能推迟的都推�?- 使用消息队列将并行任务串行来缓解服务器压力�?

   - 服务器CPU利用率出现瞬时峰�?- 削峰（CPU利用率平缓的增长�?
...
4. 用`select_related()`和`prefetch_related()`执行预加载避免�?+N查询问题”�?
...
3. 使用索引来优化查询性能（索引放在要用于查询的字段上）。InnoDB用的是BTREE索引，使�?�?�?=�?=、BETWEEN或者LIKE 'pattern'（pattern不以通配符开头）时都可以用到索引。因为建立索引需要额外的磁盘空间，而主键上是有默认的索引，所以主键要尽可能选择较短的数据类型来减少磁盘占用，提高索引的缓存效果�?
