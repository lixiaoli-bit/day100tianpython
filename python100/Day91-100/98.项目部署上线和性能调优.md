...
3. 安全相关的配置�?
   ...
   # 防止点击劫持攻击手段 - 修改HTTP协议响应�?
   # 当前网站是不允许使用<iframe>标签进行加载�?
...
2. 修改uWSGI的配置文件（`/root/project/conf/uwsgi.ini`）�?
   ...
   # 指定uwsgi文件
   module=%(name).wsgi
   # 通信的地址和端�?自己服务器的IP地址和端�?
   socket=172.18.61.250:8000
   # 日志文件地址
   logto=%(base)/logs/uwsgi.log
   ```

   > 说明：可以先将“通信的地址和端口”项等号前面改为http来进行测试，如果没有问题再改�?   成socket，然后通过Nginx来实现项目的“动静分离”（静态资源交给Nginx处理，动态内容交�?   uWSGI处理）。按照下面的方式可以启动uWSGI服务器�?
...
1. 常用开源软件�?
   ...
   | 非关系型数据�?     | MongoDB、Redis、Cassandra、TiDB |
