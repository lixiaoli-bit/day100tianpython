...
接下来，我们利用之前创建的学校选课系统数据库，为大家讲�?DML 中的查询操作。无论对于开发人员还是数据分析师，查询都是非常重要的，它关系着我们能否从关系数据库中获取我们需要的数据。建议大家把上上一节课中建库建表的 DDL 以及 上一节课中插入数据的 DML 重新执行一次，确保表和数据跟没有问题再执行下面的操作�?
...
-- 查询学分是奇数的课程的名称和学分(数据筛�?
SELECT cou_name,
           cou_credit
  FROM tb_course
 WHERE cou_credit MOD 2 <> 0;

-- 查询选择选了1111的课程考试成绩�?0分以上的学生学号(数据筛�?
SELECT stu_id
  FROM tb_record
 WHERE cou_id = 1111
       AND score > 90;

-- 查询名字叫“杨过”的学生的姓名和性别(数据筛�?
...
-- 查询学号最后一位是3的学生的学号和姓�?模糊匹配)
SELECT stu_id,
       stu_name
  FROM tb_student
 WHERE stu_id LIKE '%3';

-- 查询名字中有“不”字或“嫣”字的学生的学号和姓�?模糊匹配和并集运�?
SELECT stu_id,
       stu_name
  FROM tb_student
 WHERE stu_name LIKE '%�?'
...
-- 查询姓“杨”或姓“林”名字三个字的学生的学号和姓�?正则表达式模糊匹�?
SELECT stu_id,
       stu_name
  FROM tb_student
...
-- 查询编号�?111的课程考试成绩的最高分(聚合函数)
SELECT MAX(score)
  FROM tb_record
 WHERE cou_id = 1111;

-- 查询学号�?001的学生考试成绩的最低分、最高分、平均分、标准差、方�?聚合函数)
...
-- 查询学号�?001的学生考试成绩的平均分，如果有null值，null值算0�?聚合函数)
SELECT ROUND(SUM(score) / COUNT(*), 1) AS 平均�?
  FROM tb_record
 WHERE stu_id = 1001;
...
-- 查询每个学生的学号和平均成绩(分组和聚合函�?
SELECT stu_id AS 学号,
           ROUND(AVG(score), 1) AS 平均�?
  FROM tb_record
 GROUP BY stu_id;

-- 查询平均成绩大于等于90分的学生的学号和平均成绩(分组后的数据筛�?
SELECT stu_id AS 学号,
           ROUND(AVG(score), 1) AS 平均�?
  FROM tb_record
 GROUP BY stu_id
...
-- 查询1111�?222�?333三门课程平均成绩大于等于90分的学生的学号和平均成绩(分组前后的数据筛�?
SELECT stu_id AS 学号,
           ROUND(AVG(score), 1) AS 平均�?
  FROM tb_record
...
2. MySQL 中支持多种类型的运算符，包括：算术运算符（`+`、`-`、`*`、`/`、`%`）、比较运算符（`=`、`<>`、`<=>`、`<`、`<=`、`>`、`>=`、`BETWEEN...AND..`.、`IN`、`IS NULL`、`IS NOT NULL`、`LIKE`、`RLIKE`、`REGEXP`）、逻辑运算符（`NOT`、`AND`、`OR`、`XOR`）和位运算符（`&`、`|`、`^`、`~`、`>>`、`<<`），我们可以�?DML 中使用这些运算符处理数据�?
...
3. 在查询数据时，可以在`SELECT`语句及其子句（如`WHERE`子句、`ORDER BY`子句、`HAVING`子句等）中使用函数，这些函数包括字符串函数、数值函数、时间日期函数、流程函数等，如下面的表格所示�?
   ...
   | `FORMAT`                    | 将数值格式化成字符串并指定保留几位小�?               |
   | `FROM_BASE64` / `TO_BASE64` | BASE64解码/编码                                       |
   | `BIN` / `OCT` / `HEX`       | 将数值转换成二进�?八进�?十六进制字符�?             |
   | `LOCATE`                    | 在字符串中查找一个子串的位置                          |
   | `LEFT` / `RIGHT`            | 返回一个字符串左边/右边指定长度的字�?                |
   | `LENGTH` / `CHAR_LENGTH`    | 返回字符串的长度以字�?字符为单�?                    |
   | `LOWER` / `UPPER`           | 返回字符串的小写/大写形式                             |
   | `LPAD` / `RPAD`             | 如果字符串的长度不足，在字符串左�?右边填充指定的字�?|
   | `LTRIM` / `RTRIM`           | 去掉字符串前�?后面的空�?                            |
   | `ORD` / `CHAR`              | 返回字符对应的编�?返回编码对应的字�?                |
   | `STRCMP`                    | 比较字符串，返回-1�?�?分别表示小于、等于、大�?     |
   ...
   | -------------------------------------------------------- | ---------------------------------- |
   | `ABS`                                                    | 返回一个数的绝度�?                |
   | `CEILING` / `FLOOR`                                      | 返回一个数上取�?下取整的结果      |
   | `CONV`                                                   | 将一个数从一种进制转换成另一种进�?|
   | `CRC32`                                                  | 计算循环冗余校验�?                |
   ...
   | 函数                                      | 功能                                  |
   | ----------------------------------------- | ------------------------------------- |
   | `CURDATE` / `CURTIME` / `NOW`             | 获取当前日期/时间/日期和时�?         |
   | `ADDDATE` / `SUBDATE`                     | 将两个日期表达式相加/相减并返回结�?  |
   | `DATE` / `TIME`                           | 从字符串中获取日�?时间               |
   | `YEAR` / `MONTH` / `DAY`                  | 从日期中获取�?�?�?                 |
   | `HOUR` / `MINUTE` / `SECOND`              | 从时间中获取�?�?�?                 |
   | `DATEDIFF` / `TIMEDIFF` / `TIMESTAMPDIFF` | 返回两个时间日期表达式相差多少天/小时 |
   | `MAKEDATE` / `MAKETIME`                   | 制造一个日�?时间                     |
   ...
   | `NULLIF` | 两个表达式相等就返回NULL否则返回第一个表达式的�?|
   ...
   | `CHARSET` / `COLLATION`    | 返回字符�?校对规则           |
   | `USER` / `CURRENT_USER`    | 返回当前用户                  |
   | `DATABASE`                 | 返回当前数据库名              |
   | `VERSION`                  | 返回当前数据库版�?           |
   | `FOUND_ROWS` / `ROW_COUNT` | 返回查询到的行数/受影响的行数 |
   | `LAST_INSERT_ID`           | 返回最后一个自增主键的�?     |
   | `UUID` / `UUID_SHORT`      | 返回全局唯一标识�?           |
