...
1. 数据持久�?- 将数据保存到能够长久保存数据的存储介质中，在掉电的情况下数据也不会丢失�?

2. 数据库发展史 - 网状数据库、层次数据库、关系数据库、NoSQL 数据库、NewSQL 数据库�?

   > 1970年，IBM的研究员E.F.Codd�?Communication of the ACM*上发表了名为*A Relational Model of Data for Large Shared Data Banks*的论文，提出�?*关系模型**的概念，奠定了关系模型的理论基础。后来Codd又陆续发表多篇文章，论述了范式理论和衡量关系系统�?2条标准，用数学理论奠定了关系数据库的基础�?
...
4. ER模型（实体关系模型）和概念模型图�?
   ...
   **ER模型**，全称为**实体关系模型**（Entity-Relationship Model），由美籍华裔计算机科学家陈品山先生提出，是概念数据模型的高层描述方式，如下图所示�?
   ...
   实际项目开发中，我们可以利用数据库建模工具（如：PowerDesigner）来绘制概念数据模型，然后再设置好目标数据库系统，将概念模型转换成物理模型（如下图所示），最终生成创建二维表�?SQL（很多工具都可以根据我们设计的物理模型图以及设定的目标数据库来导�?SQL 或直接生成数据表）�?
...
5. 关系数据库产品�?
   ...
   - [DB2](https://www.ibm.com/analytics/us/en/db2/) - IBM 公司开发的、主要运行于 Unix（包�?IBM 自家�?[AIX](https://zh.wikipedia.org/wiki/AIX)）、Linux、以�?Windows 服务器版等系统的关系数据库产品。DB2 历史悠久且被认为是最早使�?SQL 的数据库产品，它拥有较为强大的商业智能功能�?
   - [SQL Server](https://www.microsoft.com/en-us/sql-server/) - �?Microsoft 开发和推广的关系型数据库产品，最初适用于中小企业的数据管理，但是近年来它的应用范围有所扩展，部分大企业甚至是跨国公司也开始基于它来构建自己的数据管理系统�?
   - [MySQL](https://www.mysql.com/) - MySQL 是开放源代码的，任何人都可以�?GPL（General Public License）的许可下下载并根据个性化的需要对其进行修改。MySQL 因为其速度、可靠性和适应性而备受关注�?
   - [PostgreSQL]() - �?BSD 许可证下发行的开放源代码的关系数据库产品�?

### MySQL 简�?

MySQL 最早是由瑞典的 MySQL AB 公司开发的一个开放源码的关系数据库管理系统，该公司于2008年被昇阳微系统公司（Sun Microsystems）收购。在2009年，甲骨文公司（Oracle）收购昇阳微系统公司，因�?MySQL 目前也是 Oracle 旗下产品�?

MySQL 在过去由于性能高、成本低、可靠性好，已经成为最流行的开源数据库，因此被广泛地应用于中小型网站开发。随着 MySQL 的不断成熟，它也逐渐被应用于更多大规模网站和应用，比如维基百科、谷歌（Google）、脸书（Facebook）、百度、淘宝、腾讯、新浪、去哪儿等都使用�?MySQL 来提供数据持久化服务�?

甲骨文公司收购后昇阳微系统公司，大幅调涨 MySQL 商业版的售价，且甲骨文公司不再支持另一个自由软件项�?[OpenSolaris ](https://zh.wikipedia.org/wiki/OpenSolaris) 的发展，因此导致自由软件社区对于 Oracle 是否还会持续支持 MySQL 社区版（MySQL 的各个发行版本中唯一免费的版本）有所担忧，MySQL 的创始人麦克尔·维德纽斯以 MySQL 为基础，创建了 [MariaDB](https://zh.wikipedia.org/wiki/MariaDB)（以他女儿的名字命名的数据库）分支。有许多原来使用 MySQL 数据库的公司（例如：维基百科）已经陆续完成了�?MySQL 数据库到 MariaDB 数据库的迁移�?
...
4. 可以�?Windows 系统的“服务”窗口中启动或停�?MySQL�?
...
下面�?CentOS 7.x 环境为例，演示如何安�?MySQL 5.7.x，如果需要在其他 Linux 系统下安装其他版本的 MySQL，请读者自行在网络上查找对应的安装教程�?

1. 安装 MySQL�?

   可以�?[MySQL 官方网站](<https://www.mysql.com/>)下载安装文件。首先在下载页面中选择平台和版本，然后找到对应的下载链接，直接下载包含所有安装文件的归档文件，解归档之后通过包管理工具进行安装�?
   ...
   接下来可以按照如下所示的顺序�?RPM（Redhat Package Manager）工具安�?MySQL�?
...
2. 配置 MySQL�?
   ...
   通过配置文件，我们可以修�?MySQL 服务使用的端口、字符集、最大连接数、套接字队列大小、最大数据包大小、日志文件的位置、日志过期时间等配置。当然，我们还可以通过修改配置文件来对 MySQL 服务器进行性能调优和安全管控�?
...
4. 使用 MySQL 客户端工具连接服务器�?
   ...
   如果是首次安�?MySQL，可以使用下面的命令来找到默认的初始密码�?
   ...
   > **说明**：MySQL 较新的版本默认不允许使用弱口令作为用户口令，所以上面的代码修改了验证用户口令的策略和口令的长度。事实上我们不应该使用弱口令，因为存在用户口令被暴力破解的风险。近年来�?*攻击数据库窃取数据和劫持数据库勒索比特币**的事件屡见不鲜，要避免这些潜在的风险，最为重要的一点是**不要让数据库服务器暴露在公网�?*（最好的做法是将数据库置于内网，至少要做到不向公网开放数据库服务器的访问端口），另外要保管好`root`账号的口令，应用系统需要访问数据库时，通常不使用`root`账号进行访问，而是**创建其他拥有适当权限的账号来访问**�?

   再次使用客户端工具连�?MySQL 服务器时，就可以使用新设置的口令了。在实际开发中，为了方便用户操作，可以选择图形化的客户端工具来连接 MySQL 服务器，包括�?
...
macOS 系统安装 MySQL 是比较简单的，只需要从刚才说到的官方网站下�?DMG 安装文件并运行就可以了，下载的时候需要根据自己使用的�?Intel 的芯片还是苹果的 M1 芯片选择下载链接，如下图所示�?

<img src="res/20211121215901.png" style="zoom:50%;">

安装成功后，可以在“系统偏好设置”中找到“MySQL”，在如下所示的画面中，可以启动和停�?MySQL 服务器，也可以对 MySQL 核心文件的路径进行配置�?
...
�?MySQL 命令行工具中，可以使用`help`命令或`?`来获取帮助，如下所示�?
...
8. 执行 SQL 文件 - `source`。`source`命令后面�?SQL 文件路径�?

9. 重定向输�?- `tee` / `notee`。可以将命令的输出重定向到指定的文件中�?
