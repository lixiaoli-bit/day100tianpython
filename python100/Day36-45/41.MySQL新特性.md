...
很多开发者在使用关系型数据库做数据持久化的时候，常常感到结构化的存储缺乏灵活性，因为必须事先设计好所有的列以及对应的数据类型。在业务发展和变化的过程中，如果需要修改表结构，这绝对是比较麻烦和难受的事情。从 MySQL 5.7 版本开始，MySQL引入了对 JSON 数据类型的支持（MySQL 8.0 解决�?JSON 的日志性能瓶颈问题），用好 JSON 类型，其实就是打破了关系型数据库和非关系型数据库之间的界限，为数据持久化操作带来了更多的便捷�?

JSON 类型主要分为 JSON 对象�?JSON数组两种，如下所示�?
...
```JSON
[{"name": "骆昊", "tel": "13122335566"}, {"name": "王大�?, "QQ": "123456"}]
```
...
```SQL
SELECT `user_id`
     , JSON_UNQUOTE(JSON_EXTRACT(`login_info`, '$.tel')) AS 手机�?
...
因为支持 JSON 类型，MySQL 也提供了配套的处�?JSON 数据的函数，就像上面用到的`json_extract`和`json_unquote`。当然，上面�?SQL 还有更为便捷的写法，如下所示�?

```SQL
SELECT `user_id`
     , `login_info` ->> '$.tel' AS 手机�?
...
`tag_name` varchar(20)  NOT NULL COMMENT '标签�?,
PRIMARY KEY (`tag_id`)
);
...
> **说明**：上面的查询用到了`member of`谓词和两�?JSON 函数，`json_contains`可以检�?JSON 数组是否包含了指定的元素，而`json_overlaps`可以检�?JSON 数组是否与指定的数组有重叠部分�?

### 窗口函数

MySQL �?.0开始支持窗口函数，大多数商业数据库和一些开源数据库早已提供了对窗口函数的支持，有的也将其称之为 OLAP（联机分析和处理）函数，听名字就知道跟统计和分析相关。为了帮助大家理解窗口函数，我们先说说窗口的概念�?
...
```SQL
<窗口函数> OVER (PARTITION BY <用于分组的列�? ORDER BY <用于排序的列�?  ROWS BETWEEN ... AND ...)
<窗口函数> OVER (PARTITION BY <用于分组的列�? ORDER BY <用于排序的列�? RANGE BETWEEN ... AND ...)
```
