...
数据库服务器通常包含了非常重要的数据，可以通过访问控制来确保这些数据的安全，�?DCL 就是解决这一问题的，它可以为指定的用户授予访问权限或者从指定用户处召回指定的权限。DCL 对数据库管理员来说非常重要，因为用户权限的管理关系到数据库的安全。简单的说，我们可以通过 DCL 允许受信任的用户访问数据库，阻止不受信任的用户访问数据库，同时还可以通过 DCL 将每个访问者的的权限最小化（让访问者的权限刚刚够用）�?

### 创建用户

我们可以使用下面�?SQL 来创建一个用户并为其指定访问口令�?

```SQL
CREATE USER 'wangdachui'@'%' IDENTIFIED BY 'Wang.618';
```

上面�?SQL 创建了名�?wangdachui 的用户，它的访问口令�?Wang.618，该用户可以从任意主机访问数据库服务器，因为 @ 后面使用了可以表示任意多个字符的通配�?%。如果要限制 wangdachui 这个用户只能�?192.168.0.x 这个网段的主机访问数据库服务器，可以按照下面的方式来修改 SQL 语句�?
...
此时，如果我们使�?wangdachui 这个账号访问数据库服务器，我们几乎不能做任何操作，因为该账号没有任何操作权限�?

### 授予权限

我们用下面的语句�?wangdachui 授予查询 school 数据库学院表（`tb_college`）的权限�?

```SQL
GRANT SELECT ON `school`.`tb_college` TO 'wangdachui'@'192.168.0.%';
```

我们也可以让 wangdachui �?school 数据库的所有对象都具有查询权限，代码如下所示�?

```SQL
GRANT SELECT ON `school`.* TO 'wangdachui'@'192.168.0.%';
```

如果我们希望 wangdachui 还有 insert、delete �?update 权限，可以使用下面的方式进行操作�?
...
如果要召�?wangdachui �?school 数据库的 insert、delete �?update 权限，可以使用下面的操作�?
