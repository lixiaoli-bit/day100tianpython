...
```python
sales_df = pd.read_excel('data/2020年销售数�?xlsx')
sales_df['月份'] = sales_df.销售日�?dt.month
sales_df['销售额'] = sales_df.售价 * sales_df.销售数�?
...
> **说明**：使�?JupyterLab 时，可以通过`DataFrame`对象的`style`属性在网页中对其进行渲染，上面的代码通过`Styler`对象的`format`方法将环比格式化为百分比进行显示，此外还指定了将空值替换为`--------`�?
...
```Python
baidu_df = pd.read_excel('data/2022年股票数�?xlsx', sheet_name='BIDU', index_col='Date')
...
在统计学中，我们通常使用协方差（covariance）来衡量两个随机变量的联合变化程度。如果变�?$\small{X}$ 的较大值主要与另一个变�?$\small{Y}$ 的较大值相对应，而两者较小值也相对应，那么两个变量倾向于表现出相似的行为，协方差为正。如果一个变量的较大值主要对应于另一个变量的较小值，则两个变量倾向于表现出相反的行为，协方差为负。简单的说，协方差的正负号显示着两个变量的相关性。方差是协方差的一种特殊情况，即变量与自身的协方差�?

$$
cov(X,Y) = E((X - \mu)(Y - \upsilon)) = E(X \cdot Y) - \mu\upsilon
$$

如果 $\small{X}$ �?$\small{Y}$ 是统计独立的，那么二者的协方差为 0，这是因为在 $\small{X}$ �?$\small{Y}$ 独立的情况下�?

$$
E(X \cdot Y) = E(X) \cdot E(Y) = \mu\upsilon
$$

协方差的数值大小取决于变量的大小，通常是不容易解释的，但是正态形式的协方差可以显示两变量线性关系的强弱。在统计学中，皮尔逊积矩相关系数就是正态形式的协方差，它用于度量两个变�?$\small{X}$ �?$\small{Y}$ 之间的相关程度（线性相关），其值介�?-1 �?1 之间�?

$$
\frac {cov(X, Y)} {\sigma_{X}\sigma_{Y}}
$$

估算样本的协方差和标准差，可以得到样本皮尔逊系数，通常用希腊字�?$\small{\rho}$ 表示�?

$$
\rho = \frac {\sum_{i=1}^{n}(X_i - \bar{X})(Y_i - \bar{Y})} {\sqrt{\sum_{i=1}^{n}(X_i - \bar{X})^2} \sqrt{\sum_{i=1}^{n}(Y_i - \bar{Y})^2}}
$$

我们�?$\small{\rho}$ 值判断指标的相关性时遵循以下两个步骤�?

1. 判断指标间是正相关、负相关，还是不相关�?
    - �?$\small{\rho \gt 0}$，认为变量之间是正相关，也就是两者的趋势一致�?
    - �?$\small{\rho \lt 0}$，认为变量之间是负相关，也就是两者的趋势相反�?
    - �?$\small{\rho \approx 0}$，认为变量之间是不相关的，但并不代表两个指标是统计独立的�?
2. 判断指标间的相关程度�?
    - �?$\small{\rho}$ 的绝对值在 $\small{[0.6,1]}$ 之间，认为变量之间是强相关的�?
    - �?$\small{\rho}$ 的绝对值在 $\small{[0.1,0.6)}$ 之间，认为变量之间是弱相关的�?
    - �?$\small{\rho}$ 的绝对值在 $\small{[0,0.1)}$ 之间，认为变量之间没有相关性�?
...
> **说明**：上面代码中使用了相对路径来访问 CSV 文件，也就是�?CSV 文件在当前工作路径下名为`data`的文件夹中。如果需要上面例子中�?CSV 文件，可以通过下面的百度云盘地址进行获取。链接：<https://pan.baidu.com/s/1rQujl5RQn9R7PadB2Z5g_g?pwd=e7b4>，提取码：e7b4�?
