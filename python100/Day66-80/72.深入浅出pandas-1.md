...
Pandas �?Wes McKinney �?008年开发的一个强大的**分析结构化数�?*的工具集。Pandas �?NumPy 为基础（实现数据存储和运算），提供了专门用于数据分析的类型、方法和函数，对数据分析和数据挖掘提供了很好的支持；同时 pandas 还可以跟数据可视化工�?matplotlib 很好的整合在一起，非常轻松愉快的实现数据可视化呈现�?
...
ser1 = pd.Series(data=[120, 380, 250, 360], index=['一季度', '二季�?, '三季�?, '四季�?])
ser1
```
...
```Python
ser2 = pd.Series({'一季度': 320, '二季�?: 180, '三季�?: 300, '四季�?: 405})
ser2
```
...
| 属�?                     | 说明                                    |
| ------------------------- | --------------------------------------- |
| `dtype` / `dtypes`        | 返回`Series`对象的数据类�?             |
| `hasnans`                 | 判断`Series`对象中有没有空�?           |
| `at` / `iat`              | 通过索引访问`Series`对象中的单个�?     |
| `loc` / `iloc`            | 通过索引访问`Series`对象中的单个值或一组�?|
...
int64
False
Index(['一季度', '二季�?, '三季�?, '四季�?], dtype='object')
[320 600 500 520]
False
True
```
...
> **说明**：`np.nan`是一个IEEE 754标准的浮点小数，专门用来表示“不是一个数”，在上面的代码中我们用它来代表空值；当然，也可以�?Python 中的`None`来表示空值，�?pandas 中`None`也会被处理为`np.nan`�?
...
```Python
ser4.fillna(method='ffill')  # 用空值前面的非空值填�?
```
...
```Python
ser8.sort_index(ascending=False)  # 按索引从大到小排�?
```
