...
通过前面的学习，我们已经对数据可视化工具 matplotlib 有一个初步的认知。大家可能也会发现了，matplotlib 提供的函数虽然强大，但是参数太多，要想对图表进行深度的定制就需要修改一系列的参数，这一点对新手并不友好。另一方面，使�?matplotlib 定制的统计图是静态图表，可能在某些需要交互效果的场景下并不合适。为了解决这两个问题，我们为大家介绍两个新的可视化工具，一个是 seaborn，一个是 pyecharts�?

### Seaborn

Seaborn 是建立在 matplotlib 之上的数据可视化工具，它相当于是�?matplotlib 进行了更高级的封装，而且 seaborn 也能�?pandas 无缝整合，让我们可以用更少的代码构建出更好的统计图表，帮助我们探索和理解数据。Seaborn 包含但不局限于以下描述的功能：
...
可以使用 Python 的包管理工具 pip 来安�?seaborn�?

```Bash
pip install seaborn
```

�?Jupyter 中，可以直接使用魔法指令进行安装，如下所示�?

```Bash
%pip install seaborn
```

下面，我们用 seaborn 自带的数据集为例，为大家简单的展示 seaborn 的用法和强大之处，想要深入研�?seaborn 的读者可以自行阅读官方[文档](https://seaborn.pydata.org/tutorial.html)和并查看官方作品集中的[示例。](https://seaborn.pydata.org/examples/index.html)根据官方示例来编写自己的代码是一个不错的选择，简单的说就是保留官方代码，将数据换成自己的数据即可。下图展示了 seaborn 绘制图表的函数，可以看出，seaborn 的这些函数主要支持我们通过绘制图表来探索数据的关系、分布和分类�?
...
如果需要在图表上显示中文，还需要用之前讲过的方法修�?matplotlib 的配置参数，代码如下所示�?
...
> **注意**：上面的代码必须放在调用 set_theme 函数之后，否则调�?set_theme 函数时又会重新修�?matplotlib 配置参数中的字体设置�?

加载官方�?Tips 数据集（就餐小费数据）�?
...
接下来，我们�?total_bill �?tip 两组数据绘制联合分布图，代码如下所示�?
...
上面清晰的展示了，total_bill �?tip 之间存在正相关关系，这一点我们也可以通过 DataFrame 对象�?corr 方法进行验证。接下来，我们可以建立回归模型来拟合这些数据点，�?seaborn 的线性回归模型图已经帮我们实现了这项功能，代码如下所示�?
...
Echarts 原来是百度开发的一个前端图表库�?018�?�?6日，ECharts 进入 Apache Incubator 进行孵化，目前已经是 Apache 软件基金会的顶级项目。凭借着良好的交互性和精巧的图表设计，ECharts 得到了众多开发者的认可，�?pyecharts 就是基于 Python 语言�?ECharts 进行了包装，�?Python 开发者也可以使用 ECharts 绘制外观精美且交互性强的统计图表�?

可以使用 Python 的包管理工具 pip 来安�?pyecharts�?

```Bash
pip install pyecharts
```

�?JupyterLab 中，可以直接使用魔法指令进行安装，如下所示�?

```Bash
%pip install pyecharts
```

如果想在 JupyterLab 中使�?pyecharts 绘图，我们还需要做一些准备工作，主要是修�?pyecharts 的配置，代码如下所示�?
...
接下来，我们通过来自�?pyecharts 官方网站新手教程中的一个例子，来认�?pyecharts。当然，我们对官网的例子进行一些调整，代码如下所示�?
...
# 设置横轴数据
bar_chart.add_xaxis(["衬衫", "羊毛�?, "雪纺�?, "裤子", "高跟�?, "袜子"])
# 设置纵轴数据（第一组）
...
在执行完上面的代码后，我们就可以通过调用`bar`对象的方法来完成对图表的渲染。如果直接使用`render`方法，那么绘制好的统计图表将保存到一�?HTML 文件中，打开该文件也能够看到绘制好的统计图表，而`render_notebook`方法则是将图表渲染到浏览器窗口中�?
...
需要提醒大家注意的是，pyecharts 并不能直接使�?NumPy �?ndarray �?Pandas �?Series、DataFrame 为其提供数据，它需要的�?Python 原生的数据类型。可能大家也注意到了，上面的代码中，我们使用的都是列表、元组这样的数据类型�?
...
data = [
    ...
    ('上海', 138), ('河北', 86), ('安徽', 79), ('湖北', 75), ('黑龙�?, 70), 
    ('陕西', 63), ('吉林', 59), ('江西', 56), ('重庆', 46), ('贵州', 39),
    ('山西', 37), ('云南', 33), ('广西', 24), ('天津', 22), ('新疆', 21),
    ('海南', 18), ('内蒙�?, 14), ('台湾', 11), ('甘肃', 7), ('广西壮族自治�?, 4),
...
�?seaborn 一样，我们建议大家参考官方提供的示例来使�?pyecharts，我们可以在 pyecharts [官方网站](https://pyecharts.org/#/zh-cn/)的左侧导航栏中找到“图表类型”选项，下面每种类型的图表都有对应的官方示例，很多代码是可以直接使用的，我们需要做的就是将数据换成自己的数据�?
