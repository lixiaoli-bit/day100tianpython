...
> **说明**：上面代码中的`mean`、`median`和`quantile`分别�?NumPy 中计算算术平均值、中位数和分位数的函数，其中`quantitle`函数的第二个参数设置�?.5表示计算50%分位数，也就是中位数�?
...
> **说明**：箱线图中的小圆圈用来表示离群点，也就是大于 $\small{Q_3 + 1.5 \times IQR}$ 或小�?$\small{Q_1 - 1.5 \times IQR}$ 的值。公式中的常�?1.5 可以通过绘制箱线图的`boxplot`函数的`whis`参数进行修改，常用的值是 1.5 �?3，修改为 3 通常是为了标识出极度离群点�?
...
print(np.mean(array1))                # 算术平均�?
print(stats.gmean(array1))            # 几何平均�?
print(stats.hmean(array1))            # 调和平均�?
print(stats.tmean(array1, [10, 90]))  # 去尾平均�?
print(stats.variation(array1))        # 变异系数
print(stats.skew(array1))             # 偏态系�?
